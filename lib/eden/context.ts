...
export function buildDocumentsContext(documents: DocContextItem[]): string {
  if (documents.length === 0) {
    return [
      "## DOCUMENTS",
      "Aucun document téléversé. Si le dirigeant mentionne un document utile (états financiers, plan d'affaires…), invite-le à le déposer dans la section Documents de son espace (PDF de préférence) pour que tu puisses l'analyser.",
    ].join("\n");
  }
